...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        if cfg['use_kfold'] ==True:
            self.log_name = self.cfg['data_name']+'/'+self.cfg['algorithm'] + '/'+ 'kfold_'+ str(cfg['fold']) + '/'  +str(self.cfg['data']['upsacle']) + '_loss_'+str(self.cfg['schedule']['loss']) + '_lambda_'+str(str(self.cfg['schedule']['auxi_lambda']) )+'_' + str(self.timestamp)
        else:
        ...
        save_config(self.log_name, 'Val dataset has {} images and {} batches.'.format(len(self.test_dataset), len(self.test_loader)))
    ...
    def run(self):
        ...
        try:
            while self.epoch <= self.nEpochs:
                ...
                elif self.epoch <=900 and self.epoch%5 ==0:
                    self.eval()
                    self.save_checkpoint()
                self.scheduler.step()
                self.epoch += 1
