...
import numpy as np
from model.modules1 import InvertibleConv1x1
...
class FeatureProcess(nn.Module):
    def __init__(self, mschannels,channels):
        super(FeatureProcess, self).__init__()

        self.conv_p = nn.Conv2d(mschannels, channels, 3, 1, 1)
...
class Net(nn.Module):
    def __init__(self, num_channels=64, args=''):
        super(Net, self).__init__()
        mschannels = args['data']['n_colors']
        out_channel = args['data']['n_colors']
        self.process = FeatureProcess(mschannels,num_channels)
        # self.cdc = nn.Sequential(nn.Conv2d(1, 4, 1, 1, 0), cdcconv(4, 4), cdcconv(4, 4))
        self.refine = Refine(num_channels, out_channel)
