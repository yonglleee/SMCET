...
import numpy as np
from modules1 import InvertibleConv1x1
...
class STFreprocess(nn.Module):
    ...
    def forward(self, msf, panf):
        ...
        s2 = 42
        # split
        msfa = msf[:, :, :s1+s2, :s1+s2] # 85 85
        msfb = msf[:, :, :s1+s2, s1:]# 85 213
        msfc = msf[:, :, s1:, :s1+s2] # 213 85
        msfd = msf[:, :, s1:, s1:]# 213 213
        ...
        outup =  torch.cat((out_list[0][:,:,:s1,:s1],
                            (out_list[0][:,:,:s1,s1:]+out_list[1][:,:,:s1,:s2]),
                            out_list[1][:, :, :s1, s2:]
                            ), dim=-1)
...
class SpaFre(nn.Module):
    ...
    def forward(self, msf, pan):  #, i
        ...
        # muti-fusion
        print(spafuse.shape,frefuse.shape,STfuse.shape)
...
class Net(nn.Module):
    def __init__(self, num_channels=4, args=''):
        super(Net, self).__init__()
