...
LastEditTime: 2021-12-19 14:01:05
...
class Net(nn.Module):
    ...
    def forward(self, l_ms, b_ms, x_pan):
        ...
        one = torch.ones_like(NDWI)
        NDWI = torch.where(torch.isnan(NDWI), one, NDWI)

        NDWI = F.interpolate(NDWI, scale_factor=self.args['data']['upsacle'], mode='bicubic')
        NDVI = ((l_ms[:, 3, :, :] - l_ms[:, 2, :, :]) / (l_ms[:, 3, :, :] + l_ms[:, 2, :, :])).unsqueeze(1)
        # NDVI异常值
        one = torch.ones_like(NDVI)
        NDVI = torch.where(torch.isnan(NDVI), one, NDVI)
