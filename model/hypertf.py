...
class Net(nn.Module):
    def __init__(self, args):
        ...
        self.TS_lv3 = MultiHeadAttention(n_head=int(n_head), in_pixels=int(lv1_pixels),
                                         linear_dim=int(args['data']['patch_size']), num_features=self.n_feats)
        self.TS_lv2 = MultiHeadAttention(n_head=int(n_head), in_pixels=int(lv2_pixels),
                                         linear_dim=int(args['data']['patch_size']), num_features=int(self.n_feats / 2))
        self.TS_lv1 = MultiHeadAttention(n_head=int(n_head), in_pixels=int(lv3_pixels),
                                         linear_dim=int(args['data']['patch_size']), num_features=int(self.n_feats / 4))
        ...
        for i in range(self.num_res_blocks[1]):
            self.RB11.append(ResnetBlock(input_size=self.n_feats, scale=self.res_scale))
        self.conv11_tail = ConvBlock(self.n_feats, self.n_feats)
        ...
        for i in range(self.num_res_blocks[2]):
            self.RB22.append(ResnetBlock(input_size=int(self.n_feats / 2), scale=self.res_scale))
        self.conv22_tail = ConvBlock(int(self.n_feats / 2), int(self.n_feats / 2))
        ...
        for i in range(self.num_res_blocks[3]):
            self.RB33.append(ResnetBlock(input_size=int(self.n_feats / 4), scale=self.res_scale))
        self.conv33_tail = ConvBlock(int(self.n_feats / 4), int(self.n_feats / 4))
        ...
        for i in range(self.num_res_blocks[4]):
            self.RBF.append(ResnetBlock(input_size=self.out_channels, scale=self.res_scale))
        self.convF_tail = ConvBlock(self.out_channels, self.out_channels)
